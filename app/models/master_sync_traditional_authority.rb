class MasterSyncTraditionalAuthority < ActiveRecord::Base
  ...
  def self.people_to_push(district,traditional_authority,last_updated_datetime = nil)
    search_district = 'state_province":"' + district
    ...
    if not last_updated_datetime.blank?                                       
      ...
      AND data LIKE (?) AND people.updated_at > ?","%#{search_district}%","%#{search_traditional_authority}%",
      last_updated_datetime).select('people.*,national_patient_identifiers.value').order(:id).limit(20)
                                                                              
      self.check_for_valid_start_date(district,traditional_authority) unless people.blank?
      return people                     
    else                                                                      
      people = Person.joins(:national_patient_identifier).where("data LIKE (?) 
      AND data LIKE (?)","%#{search_district}%",
      "%#{search_traditional_authority}%").select('people.*,national_patient_identifiers.value').order(:id).limit(20)
